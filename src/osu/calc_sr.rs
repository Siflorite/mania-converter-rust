...
use crate::osu::helper_functions::*;
...
fn preprocess(osu_data: &OsuDataLegacy, speed: f64) -> PreprocessResult {
    ...
    if osu_data.misc.circle_size == 0 {
        return Err(io::Error::new(
            ...
            "Circle size is 0, meaning no columns!",
        ));
    }

    let mut note_seq: Vec<(u32, u32, i32)> = osu_data
        .notes
        .iter()
        .map(|note| {
            ...
            let t = note
                .end_time
                .map_or(-1, |x| (x as f64 * time_multiplier) as i32);
            (k, h, t)
        })
        .collect();
    ...
    // Process long notes
    let ln_seq: Vec<(u32, u32, u32)> = note_seq
        .iter()
        .filter(|&&(_, _, t)| t >= 0)
    ...
    tail_seq.sort_by(|a, b| a.2.cmp(&b.2).then(a.1.cmp(&b.1)).then(a.0.cmp(&b.0)));
...
fn get_corners(total: u32, note_seq: &[(u32, u32, i32)]) -> (Vec<u32>, Vec<u32>, Vec<u32>) {
    ...
    let corners_all_set: HashSet<u32> = corners_base_set
        .iter()
...
fn compute_anchor(_col: u32, key_usage_400: &[Vec<f64>], base_corners: &[u32]) -> Vec<f64> {
    ...
    for (idx, _) in base_corners.iter().enumerate() {
        ...
        let mut counts: Vec<f64> = key_usage_400.iter().map(|row| row[idx]).collect();
...
fn ln_sum(a: u32, b: u32, ln_rep: (&[u32], &[f64], &[f64])) -> f64 {
    ...
    let find_index = |x| points.partition_point(|&p| p <= x).saturating_sub(1);
    let i = find_index(a);
    let j = find_index(b);
...
fn compute_j_bar(
    ...
    let jack_nerfer = |delta: f64| 1.0 - 7e-5 * (0.15 + (delta - 0.08).abs()).powi(-4);
...
fn compute_x_bar(
    ...
    for k in 0..=col_u {
        ...
        for pair in notes.windows(2) {
            ...
            let should_adjust = (k == 0)
                || (k > 0
                    && !active_columns[left_idx].contains(&(k as u32 - 1))
                    && !active_columns[right_idx].contains(&(k as u32 - 1)))
                || (!active_columns[left_idx].contains(&(k as u32))
                    && !active_columns[right_idx].contains(&(k as u32)));
    ...
    for i in 0..corner_n {
        // 第一部分求和
        let part1: f64 = (0..=col_u)
            .map(|k| x_ks[k][i] * cross_coeff.get(k).unwrap_or(&0.0))
            .sum();

        // 第二部分求和
        let part2: f64 = (0..col_u)
            .map(|k| {
...
fn compute_r_bar(
    ...
    let times_by_column: Vec<Vec<u32>> = note_seq_by_column
        .iter()
        .map(|column| { column.iter().map(|note| note.1) }.collect())
        .collect();

    let mut i_list: Vec<f64> = Vec::with_capacity(tail_seq.len());
    for &(k, h_i, t_i) in tail_seq.iter() {
        let (_, h_j, _) = find_next_note_in_column(
            (k, h_i, t_i as i32),
            &times_by_column[k as usize],
            note_seq_by_column,
        );
...
fn compute_c_and_ks(
    ...
    let c_step: Vec<f64> = base_corners
        .iter()
        .map(|&t| {
    ...
    let ks_step = (0..corners_len)
        .map(|i| {
            let count = key_usage.iter().filter(|&column| column[i]).count().max(1);
            count as f64
        })
        .collect();
...
pub fn calculate_from_data(data: &OsuDataLegacy, speed: f64) -> io::Result<f64> {
    ...
    let active_columns: Vec<Vec<u32>> = (0..corners_base.len())
        .map(|i| (0..k).filter(|&c| key_usage[c as usize][i]).collect())
        .collect();
    ...
    let p_bar = compute_p_bar(
        k,
        t,
        x,
        &note_seq,
        (&ln_rep.0, &ln_rep.1, &ln_rep.2),
        &anchor,
        &base_corners_f,
    );
    ...
    // 计算最终难度指标
    let s_all: Vec<f64> = (0..all_corners_f.len())
        .map(|i| {
        ...
        .map(|v| v.powf(2.0 / 3.0))
        .collect();

    let t_all: Vec<f64> = (0..all_corners_f.len())
        .map(|i| {
            (a_bar_interp[i].powf(3.0 / ks_arr[i]) * x_bar_interp[i])
                / (x_bar_interp[i] + s_all[i] + 1.0)
        })
        .collect();

    let d_all: Vec<f64> = s_all
        .iter()
        .zip(t_all.iter())
        .map(|(s, t)| 2.7 * s.powf(0.5) * t.powf(1.5) + s * 0.27)
        .collect();

    let mut gaps = vec![0.0; all_corners_f.len()];
    if all_corners_f.len() > 1 {
        gaps[0] = (all_corners_f[1] - all_corners_f[0]) / 2.0;
        gaps[all_corners_f.len() - 1] =
            (all_corners_f[all_corners_f.len() - 1] - all_corners_f[all_corners_f.len() - 2]) / 2.0;
    ...
    // 按照 d 值排序
    let mut sorted_data: Vec<(f64, f64)> = d_all
        .iter()
    ...
    let indices: Vec<usize> = targets
        .iter()
        .map(|t| cum_weights.partition_point(|x| x < t))
        .collect();

    // 计算最终 SR
    let percentile_93: f64 = indices[0..4].iter().map(|&i| sorted_data[i].0).sum::<f64>() / 4.0;

    let percentile_83: f64 = indices[4..8].iter().map(|&i| sorted_data[i].0).sum::<f64>() / 4.0;

    let weighted_mean: f64 = (sorted_data
        .iter()

...
pub fn generate_info_abstract(
    info_vec: &[BeatMapInfo],
    temp_dir_path: &Path,
    save_pic_path: &Path,
) -> io::Result<PathBuf> {
    let card_vec: Vec<CardData> = info_vec
        .iter()
        .enumerate()
        .map(|(i, info)| {
            ...
            let title_ascii: &str = if &info.title == title {
                ""
            } else {
                &info.title
            };
            let artist = info.artist_unicode.as_ref().unwrap_or(&info.artist);
            let artist_ascii: &str = if &info.artist == artist {
                ""
            } else {
                &info.artist
            };
            ...
            let note_str = format!(
                "{} ({:.02}%)",
                info.note_count,
                info.note_count as f64 / total_count as f64 * 100.0
            );
            let ln_str = format!(
                "{} ({:.02}%) = {}",
                info.ln_count,
                info.ln_count as f64 / total_count as f64 * 100.0,
                total_count
            );
...
fn format_bpm_str(min_bpm: f64, max_bpm: Option<f64>) -> String {
    ...
    let min_bpm_str = format!("{:.1}", min_bpm)
        .trim_matches('0')
        .trim_matches('.')
        .to_string();

...
```rust
use mania_converter::malody::{process_mcz_file, process_whole_dir_mcz};
use std::io;
use std::path::Path;

pub fn main() -> io::Result<()> {
    let p = Path::new("./tests/beatmaps/Kuukan Souzou Riron.mcz");
    ...
    let (osz_dir, info_vec) = process_mcz_file(p, calc_sr).unwrap();
    println!("{:?}", info_vec);
    
    // You can use either "" or "." as the first parameter to indicate the working directory.
    // Then the compiled binary file will convert all mcz files under this directory.
